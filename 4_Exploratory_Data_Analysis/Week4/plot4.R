...
summarised <- NEI %>%
  ...
  filter(str_detect(EI.Sector, "^Fuel Comb .* Coal$")) %>%  # allows filtering for all combustion activities involving coal
  group_by(year) %>% 
  summarise(emissions = sum(Emissions), .groups = "drop")

p <- ggplot(summarised, aes(year, emissions)) + 
  geom_line() + 
  labs(title = "Evolution of PM2.5 Emissions from Coal Combustion \nin the US between 1999 and 2008", 
       x = "Year", 
       y = "PM2.5 Emissions") + 
  theme_bw()
