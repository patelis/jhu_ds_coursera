...
cities <- tibble(fips = c("24510", "06037"), city = c("Baltimore City", "Los Angeles County"))

# Summarise the data

summarised <- NEI %>%
  filter(fips %in% c("24510", "06037")) %>% # filter for Baltimore City amd Los Angeles County fips
...
p <- ggplot(summarised, aes(year, emissions, colour = city)) + 
  geom_line() + 
...
png(filename = paste0(path, "/", "plot6.png"), width = 480, height = 480, units = "px")
